...
class CaseFoldersVoter extends Voter
{
    /**
    ...
    protected function supports(string $attribute, $subject): bool
    {
        ...
        if (!in_array($attribute, ['view', 'edit', 'other_action'])) {
            return false;
        }

        // Only vote on `CaseFolder` objects
        if (!$subject instanceof CaseFolder) {
            return false;
        }
     ...
     * @param userMail $subject The subject being checked.
    ...
    {
        $userMail = $token->getUser()->getUserIdentifier();
        ...
        switch ($attribute) {
            case 'view':
                if ($userMail === $subject->getUser()->getUserIdentifier()) {
                    return true;
                }
                break;
            case 'edit':
